...
test('validate the number input to feild', async ({ inputPage }) => {
...
test('validate the number increment of the feild by arrow', async ({ inputPage }) => {
    await inputPage.enterValue('1');
    await inputPage.incrementNumber(2);
    expect(await inputPage.getEnteredValue()).toBe('3');
});

test('validate the number decrement of the feild by arrow', async ({ inputPage }) => {
    await inputPage.enterValue('4');
    await inputPage.decrementNumber(2);
    expect(await inputPage.getEnteredValue()).toBe('2');
});

test('validate the character input to feild', async ({ inputPage }) => {
